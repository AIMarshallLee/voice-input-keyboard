...
R35 compatibility ruling after actual RED #180: if another cancellation file is malformed or identity-invalid, its possible replacement reference cannot be ruled out. The pre-R33 mixed-fixture test `testNewSettingsGCPreservesMalformedOrInvalidCancellationEvidenceButExpiresValidMarkers` must therefore retain its valid expired proof while those unknown fixtures remain, then remove only the test-created invalid fixtures and assert ordinary expiry cleanup resumes. Preserve all invalid-file byte assertions and add the retained-proof byte assertion; do not merely delete the cleanup expectation. This is conservative identity-metadata retention, not transcript retention. The cost is retaining otherwise unrelated UUID/timestamp proof while an unknown potential anchor remains; unsafe deletion costs lost recovery identity. No claim is made that this adjusted compatibility assertion had a separate RED run.

**R36 — result-only payload is not durable handoff settlement:** Round2 review at `3fc8d00` found that output publication can return `.ioFailure` after result-write success, receipt-write failure and failed result rollback. The remaining decodable result alone therefore cannot release global handoff recovery or permit source-anchor GC, whether fresh or expired. `replacementIsConfirmedFinishedUncoordinated` must accept only valid identity-matched cancellation or terminal receipt as settlement. A fresh result-only payload remains eligible for existing manual/held recovery; successful consumption may establish the ordinary receipt. Expired payload still follows its existing five-minute cleanup rule, while the unresolved source retains the exact replacement identity until durable cancellation or receipt exists. Add focused real-file regressions before the minimal helper correction: fresh result-only restores the exact replacement without settling its anchor; aged result-only plus an expired source cannot retire the source, and ordinary payload cleanup cannot turn recovery into no work. Confirm successful receipt creation/cancellation releases the barrier. These fixtures represent a reachable on-disk state, not executed receipt-write/rollback-failure injection. No new persisted record, retention extension, interface, fault framework or UI feature. Cost if omitted: failed publication can discard recovery identity and admit a fresh request without confirming the replacement.
...
- source cancellation records can include only the replacement UUID as a durable handoff identity; unresolved linked records outlive ordinary cancellation TTL until a valid replacement cancellation or terminal receipt exists, including the consumed-pending/no-live recovery gap. Result-only payload cannot settle the link; fresh text remains held-recoverable and ordinary expired-text cleanup does not erase the linked identity;
